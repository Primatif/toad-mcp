...
use rmcp::model::{Implementation, ProtocolVersion, ServerCapabilities, ServerInfo, CallToolResult, Content, ErrorData as McpError};
use rmcp::{tool_handler, tool, tool_router};
use async_trait::async_trait;
use serde::Deserialize;
use schemars::JsonSchema;
use toad_core::{Workspace, GlobalConfig};
use std::fs;
...
impl ToadService {
    ...
    pub async fn list_projects(&self, params: rmcp::handler::server::wrapper::Parameters<ListProjectsParams>) -> Result<CallToolResult, McpError> {
        let params = params.0;
        
        let q_filter: Option<String> = params.query.map(|s| s.to_lowercase());
        let t_filter: Option<String> = params.tag.map(|s| if s.starts_with('#') { s.to_lowercase() } else { format!("#{}", s.to_lowercase()) });
        ...
        let result = tokio::task::spawn_blocking(move || {
            let ws = Workspace::discover().map_err(toad_core::ToadError::from)?;
            let registry = toad_core::ProjectRegistry::load(ws.active_context.as_deref(), None)
                .map_err(toad_core::ToadError::from)?;
            
            let filtered: Vec<_> = registry.projects.into_iter().filter(|p| {
                ...
                if let Some(t) = &t_filter {
                    if !p.tags.iter().any(|tag| tag.to_lowercase() == *t) {
                        return false;
                    }
                }
                if let Some(s) = &s_filter {
                    if !p.stack.to_lowercase().contains(s) {
                        return false;
                    }
                }
                if let Some(a) = &a_filter {
                    if !p.activity.to_string().to_lowercase().contains(a) {
                        return false;
                    }
                }
                if let Some(v) = &v_filter {
                    if !p.vcs_status.to_string().to_lowercase().contains(v) {
                        return false;
                    }
                }
                true
        ...
        }).await.map_err(|e| crate::errors::toad_error_to_mcp(toad_core::ToadError::Other(e.to_string())))?
    ...
    pub async fn get_project_detail(&self, params: rmcp::handler::server::wrapper::Parameters<GetProjectDetailParams>) -> Result<CallToolResult, McpError> {
        ...
        let result = tokio::task::spawn_blocking(move || {
            ...
            let project = registry.projects.iter().find(|p| p.name == name)
            ...
            if context_md_path.exists() {
                if let Ok(content) = fs::read_to_string(context_md_path) {
    ...
    pub async fn get_project_dna(&self, params: rmcp::handler::server::wrapper::Parameters<GetProjectDetailParams>) -> Result<CallToolResult, McpError> {
        let name = params.0.name;
    ...
    pub async fn compare_projects(&self, params: rmcp::handler::server::wrapper::Parameters<CompareProjectsParams>) -> Result<CallToolResult, McpError> {
        ...
        let result = tokio::task::spawn_blocking(move || {
            ...
            let proj_a = registry.projects.iter().find(|p| p.name == source)
                .ok_or_else(|| toad_core::ToadError::Other(format!("Source project '{}' not found", source)))?;
            let proj_b = registry.projects.iter().find(|p| p.name == target)
                .ok_or_else(|| toad_core::ToadError::Other(format!("Target project '{}' not found", target)))?;
    ...
    pub async fn search_projects_by_dna(&self, params: rmcp::handler::server::wrapper::Parameters<SearchProjectsParams>) -> Result<CallToolResult, McpError> {
        ...
        let result = tokio::task::spawn_blocking(move || {
            ...
            let matches: Vec<_> = registry.projects.into_iter().filter(|p| {
                p.dna.roles.iter().any(|r| r.to_lowercase().contains(&query))
                    || p.dna.capabilities.iter().any(|c| c.to_lowercase().contains(&query))
                    || p.dna.structural_patterns.iter().any(|sp| sp.to_lowercase().contains(&query))
            }).collect();
    ...
    pub async fn search_projects(&self, params: rmcp::handler::server::wrapper::Parameters<SearchProjectsParams>) -> Result<CallToolResult, McpError> {
    ...
    pub async fn get_ecosystem_summary(&self, params: rmcp::handler::server::wrapper::Parameters<GetEcosystemSummaryParams>) -> Result<CallToolResult, McpError> {
        ...
        let result = tokio::task::spawn_blocking(move || {
            ...
            let config = toad_core::GlobalConfig::load(None).map_err(toad_core::ToadError::from)?.unwrap_or_default();
    ...
    pub async fn get_ecosystem_status(&self, params: rmcp::handler::server::wrapper::Parameters<GetEcosystemStatusParams>) -> Result<CallToolResult, McpError> {
        let query = params.0.query;
        let tag = params.0.tag;
    ...
    pub async fn get_project_stats(&self, params: rmcp::handler::server::wrapper::Parameters<GetProjectStatsParams>) -> Result<CallToolResult, McpError> {
        let query = params.0.query;
        let tag = params.0.tag;
        ...
        let result = tokio::task::spawn_blocking(move || {
            ...
            let report = toad_ops::stats::generate_analytics_report(&registry.projects, query.as_deref(), tag.as_deref());
            Ok::<_, toad_core::ToadError>(serde_json::to_string_pretty(&report)?)
        }).await.map_err(|e| crate::errors::toad_error_to_mcp(toad_core::ToadError::Other(e.to_string())))?
        .map_err(crate::errors::toad_error_to_mcp)?;
    ...
    pub async fn get_active_context(&self, _params: rmcp::handler::server::wrapper::Parameters<NoParams>) -> Result<CallToolResult, McpError> {
        let result = tokio::task::spawn_blocking(move || {
            let config = GlobalConfig::load(None).map_err(toad_core::ToadError::from)?.unwrap_or_default();
            let active = config.active_context.unwrap_or_else(|| "default".to_string());
    ...
    pub async fn list_contexts(&self, _params: rmcp::handler::server::wrapper::Parameters<NoParams>) -> Result<CallToolResult, McpError> {
        let result = tokio::task::spawn_blocking(move || {
            let config = GlobalConfig::load(None).map_err(toad_core::ToadError::from)?.unwrap_or_default();
            let active = config.active_context.clone().unwrap_or_else(|| "default".to_string());
    ...
    pub async fn switch_context(&self, params: rmcp::handler::server::wrapper::Parameters<SwitchContextParams>) -> Result<CallToolResult, McpError> {
        let name = params.0.name;
        
        let result = tokio::task::spawn_blocking(move || {
            let mut config = GlobalConfig::load(None).map_err(toad_core::ToadError::from)?.unwrap_or_default();
            ...
            config.save(None).map_err(toad_core::ToadError::from)?;
